...
class ReservoirNode(mdp.Node):
    ...
    def _execute(self, x):
        ...
        # Instrinsic Plasticity
        # eta1 = main.var_eta
        # mu1 = main.var_mu
        # sigma1 = main.var_sigma
        # [ip_a, ip_b] = IP_search.IP_searching_function(X=x, W_in=self.w_in, W_hat=self.w, eta=eta1, mu=mu1, sigma=sigma1, nepochs=10)
        [ip_a, ip_b] = IP_search.IP_searching_function(X=x, W_in=self.w_in, W_hat=self.w, eta=1e-12, mu=-0.13,
                                                       sigma=0.12, nepochs=10)
        ...
        for n in range(steps):
            ################## tanh(x)-> tanh(a.*x+b) ###############################################################
            # cc = mdp.numx.dot(self.w_in, x[n, :]) + mdp.numx.dot(self.w, states[n, :]) + self.w_bias
            # dd = multiply(ip_a, cc.T) + ip_b    #dd = multiply(ip_a, cc.T) + ip_b
            # ddd = dd.T
            # states[n+1, :] = nonlinear_function_pointer(ddd )+ self.w_bias

            ################## tanh(x)-> tanh(a.*x+b) for Xabier input weight ###############################################################
            cc = mdp.numx.dot(self.w_in, x[n, :]) + mdp.numx.dot(self.w, states[n, :]) + self.w_bias
            dd = multiply(ip_a, cc.T) + ip_b
            ddd = dd.T
            states[n + 1, :] = (1 - self.spectral_radius) * states[n, :] + self.spectral_radius * np.tanh(
                ddd) + self.w_bias
            #########################################################################################################
            ################################# reservoir states = (1-a)state+a*tan(x) #################################
            # states[n + 1, :] = (1-self.spectral_radius)*states[n, :]+self.spectral_radius*np.tanh(mdp.numx.dot(self.w_in, x[n, :])+mdp.numx.dot(self.w, states[n, :])+self.w_bias)
            ############################ Original code ###############################################################
